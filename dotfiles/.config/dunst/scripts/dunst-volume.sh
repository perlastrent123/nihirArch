...
if [[ $volume == 0 || "$mute" == "yes" ]]; then
    ...
    dunstify -t 1000 -a "changeVolume" -u low -i "$HOME/.config/dunst/img/volume-m.svg" -h string:x-dunst-stack-tag:$msgTag "Volume muted"
else
    # Show the volume notification
    dunstify -t 1000 -a "changeVolume" -u low -i "$HOME/.config/dunst/img/volume-u.svg" -h string:x-dunst-stack-tag:$msgTag \
